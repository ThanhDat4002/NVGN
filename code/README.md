...
| **Nút mở tay Cổng Vào** | Một chân nút | **GPIO6** | Nhấn giữ **2 giây** để mở cổng vào |
| **Nút mở tay Cổng Ra** | Một chân nút | **GPIO7** | Nhấn giữ **2 giây** để mở cổng ra sau khi thu tiền mặt |
...
## 4. Biên dịch và nạp chương trình bằng VSCode PlatformIO

1. Mở phần mềm **VSCode** đã cài sẵn extension **PlatformIO IDE**.
2. Chọn **File -> Open Folder...** và mở thư mục chứa mã nguồn: `d:\NVGN\smart-parking-iot-dashboard\code`.
3. Nhìn xuống thanh trạng thái dưới cùng của VSCode:
   - Nhấn vào biểu tượng **Build** (hình dấu tích `✓`) để biên dịch code.
   - Nhấn vào biểu tượng **Upload** (hình mũi tên sang phải `→`) để nạp chương trình xuống chip ESP32-S3.
   - Nhấn vào biểu tượng **Serial Monitor** (hình ổ cắm điện) để mở màn hình giám sát Serial tốc độ **115200**.
4. Khi chạy lần đầu, phân vùng LittleFS sẽ tự động được định dạng và sẵn sàng hoạt động Local-First.
